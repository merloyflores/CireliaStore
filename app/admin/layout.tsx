...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-dvh bg-zinc-50 flex flex-col min-[1195px]:flex-row font-sans">
      
      {/* HEADER FLOTANTE (Visible hasta 1194px) */}
      <div className="min-[1195px]:hidden flex items-center justify-between bg-white border-b border-zinc-200 p-4 sticky top-0 z-40">
      ...
      {/* OVERLAY OSCURO (Oculto a partir de 1195px) */}
      {isMobileMenuOpen && (
        <div 
          className="fixed inset-0 bg-zinc-950/40 backdrop-blur-sm z-40 min-[1195px]:hidden transition-opacity"
          onClick={() => setIsMobileMenuOpen(false)}
        />
      )}

      {/* SIDEBAR (Flotante < 1195px, Fijo >= 1195px) */}
      <aside className={`
        fixed min-[1195px]:sticky top-0 left-0 z-50 h-dvh bg-white border-r border-zinc-100 flex flex-col transition-all duration-300 ease-in-out
        w-64 ${isSidebarOpen ? 'min-[1195px]:w-64' : 'min-[1195px]:w-20'}
        ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full min-[1195px]:translate-x-0'}
      `}>
        {/* Header del Sidebar */}
        <div className="p-6 border-b border-zinc-100 flex justify-between items-center shrink-0 h-18.25">
          <Link 
            href="/admin" 
            onClick={() => setIsMobileMenuOpen(false)}
            className={`text-2xl font-black text-zinc-950 tracking-tighter ${!isSidebarOpen ? 'min-[1195px]:hidden' : ''}`}
          >
            Cirelia<span className="text-sky-600">.</span>
          </Link>
          
          {!isSidebarOpen && (
            <div className="hidden min-[1195px]:flex w-full justify-center">
          ...
          <button 
            ...
            className="hidden min-[1195px]:block p-1.5 text-zinc-400 hover:bg-zinc-100 rounded-lg transition absolute -right-3 bg-white border border-zinc-200"
          >
          ...
          <button 
            onClick={() => setIsMobileMenuOpen(false)} 
            className="min-[1195px]:hidden p-1.5 text-zinc-400 hover:bg-zinc-100 rounded-lg transition"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-4 space-y-2 custom-scrollbar">
          <p className={`text-[10px] font-black text-zinc-400 uppercase tracking-widest px-2 mb-4 ${!isSidebarOpen ? 'min-[1195px]:hidden' : ''}`}>
            Menú Principal
          </p>
            ...
            return (
              <Link 
                ...
                className={`flex items-center py-3 rounded-xl font-bold text-sm transition-all duration-200 
                  ${!isSidebarOpen ? 'min-[1195px]:justify-center px-3' : 'px-3'}
              ...
              >
                ...
                <span className={`ml-3 ${!isSidebarOpen ? 'min-[1195px]:hidden' : ''}`}>
                  {link.name}
                </span>
              </Link>
            );
