...
export default function Home() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      // Traemos todos los productos para agruparlos correctamente
  ...
  // 1. AGRUPACIÓN DINÁMICA: Usando los nombres reales que vengan en los productos
  const groupedProducts = products?.reduce((acc: Record<string, any[]>, product) => {
    // Usamos trim() por si por error se guardó un espacio extra en Supabase
  ...
  // 2. EXTRAER DEPORTES: Buscamos 'Deportes' (tal cual está en tu imagen de Supabase)
  const randomSportsProducts = (() => {
    const sportsItems = groupedProducts['Deportes'] || groupedProducts['deportes'] || [];
    return [...sportsItems].sort(() => Math.random() - 0.5);
  })();

  // 3. CATEGORÍAS A MOSTRAR: Sacamos todas las demás, EXCLUYENDO 'Deportes' para que no se duplique
  const displayCategories = Object.keys(groupedProducts).filter(
    (cat) => cat.toLowerCase() !== 'deportes'
  );
  ...
  return (
    <div className="flex flex-col w-full">
      ...
      {/* 2. SECCIÓN CON FONDO #F3F3F4 - BLINDADA Y PRO */}
          ...
          {displayCategories.length > 0 ? (
            displayCategories.map((categoryName, index) => (
              <div key={categoryName}>
                {/* Renderiza de forma automática cada categoría real */}
                <CategoryRow category={categoryName} products={groupedProducts[categoryName]} />
                
                {/* Inserta el banner publicitario y DEPORTES justo después de la primera fila */}
                {index === 0 && (
                  <>
                    <PromoBanner 
                      image="/banner.png" 
                    ...
                    {/* INYECCIÓN DE DEPORTES: Aparece aquí si hay productos en la BD */}
            ...
            /* Fallback PRO: Si por alguna razón SOLO tenés productos de Deportes en la base de datos */
            <div>
              <PromoBanner 
                image="/banner.png" 
                title="Descuentos Goleadores" 
                ...
                link="/shop?category=Deportes"
              />
              <CategoryRow category="Deportes" products={randomSportsProducts} />
            </div>
          ) : (
            /* Estado mientras cargan los datos de Supabase */
      ...
      <section className="py-24 bg-zinc-950 text-white overflow-hidden">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { text: "Cirelia no solo cambió el look de mi sala, cambió cómo nos sentimos al llegar a casa. La calidad es inigualable.", name: "María Fernández", role: "Diseñadora de Interiores", bg: "bg-[url('https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&q=80&w=200')]" },
