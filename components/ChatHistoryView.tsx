      ...
      {chats.length === 0 ? (
        ...
        chats.map((chat) => (
          <div key={chat.id || chat.created_at} className="p-5 bg-white border border-zinc-200 rounded-2xl shadow-sm hover:border-zinc-300 transition-colors flex flex-col gap-3">
            ...
            {/* Cuerpo: Mensaje Inicial Real */}
            <div className="bg-zinc-50 border border-zinc-100 p-4 rounded-xl">
              <p className="text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-1 text-[9px]">
                Mensaje inicial:
              </p>
            ...
            {/* Footer de la tarjeta: Estado del Chat extraído de tu base de datos */}
            {chat.status && (
