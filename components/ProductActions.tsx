...
export default function ProductActions({ product }: { product: any }) {
  ...
  // Si explícitamente viene 0 stock, lo respetamos. Si es undefined o null, dejamos el fallback.
  const stockDisponible = product.stock !== undefined && product.stock !== null ? product.stock : 10;
    ...
    <div className="flex flex-col gap-5 mb-10 w-full">
        ...
        {cartItem ? (
          ...
          <div className="flex flex-col gap-3 animate-in fade-in slide-in-from-bottom-2 duration-500">
            ...
            <div className="flex items-center gap-4 w-full">
              ...
              <div className="flex items-center bg-zinc-100 rounded-2xl p-1.5 h-16 w-40 shrink-0 shadow-inner">
                ...
                <button 
                  disabled={cartItem.quantity >= stockDisponible}
                  onClick={() => addToCart(product)}
                  className="flex-1 flex items-center justify-center bg-white shadow-sm hover:shadow text-zinc-600 rounded-xl h-full transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  <Plus size={16} />
                </button>
              </div>
          ...
          <div className="flex flex-col gap-4 animate-in fade-in duration-500">
            <div className="flex items-center gap-4">
              ...
              <div className={`flex items-center bg-zinc-100 rounded-2xl p-1.5 h-16 w-36 shrink-0 shadow-inner ${isOutOfStock ? 'opacity-40 pointer-events-none' : ''}`}>
                <button 
                  disabled={isOutOfStock || localQuantity <= 1}
                  onClick={() => setLocalQuantity(Math.max(1, localQuantity - 1))} 
                  className="flex-1 flex items-center justify-center bg-white shadow-sm hover:shadow text-zinc-600 rounded-xl h-full transition-all disabled:opacity-50"
                >
                  <Minus size={16}/>
                </button>
                <span className="flex-1 text-center font-black text-lg text-zinc-900">
                  {isOutOfStock ? 0 : localQuantity}
                </span>
                <button 
                  disabled={isOutOfStock || localQuantity >= stockDisponible}
              ...
              {/* Botón Principal (Añadir / Agotado) */}
              <button 
                ...
                className={`flex-1 h-16 rounded-2xl font-black flex items-center justify-center gap-3 transition-all text-lg border ${
                  isOutOfStock 
                    ? 'bg-zinc-200 text-zinc-400 border-zinc-300 shadow-none cursor-not-allowed active:scale-100' 
                    : 'bg-zinc-950 text-white border-transparent hover:bg-sky-600 shadow-xl hover:shadow-sky-600/30 active:scale-95'
                }`}
              >
                {isOutOfStock ? (
                  <>
      ...
      <div className="flex gap-4 w-full pt-1">
        ...
        <button 
          ...
          className={`flex-1 h-16 rounded-2xl font-bold flex items-center justify-center gap-3 transition-all active:scale-95 ${
            isOutOfStock 
              ? 'bg-zinc-100 text-zinc-600 border border-zinc-200 hover:bg-zinc-200/70'
              : 'bg-[#25D366]/10 text-[#1da851] border border-[#25D366]/20 hover:bg-[#25D366]/20'
          }`}
        >
          ...
          {isOutOfStock ? (
            <>
              <span className="hidden sm:inline">Consultar Próximo Ingreso</span>
