...
export default function WhatsAppWidget({ phoneNumber }: { phoneNumber?: string }) {
  ...
  // Los returns condicionales van después de todos los hooks (regla de hooks).
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    // support_chats.tenant_id es obligatorio y este widget corre en el
    // cliente (no tiene el header x-tenant-slug que arma el proxy para
    // Server Components), así que resolvemos el tenant acá por slug,
    // igual que en /profile/soporte.
    ...
    if (tenant) {
      const { error } = await supabase
        .from('support_chats')
        .insert([{ tenant_id: tenant.id, client_name: name || 'Cliente Web', initial_message: message }]);
      if (error) console.error('No se pudo registrar el lead de soporte:', error);
    }

    const textFormatted = `Hola, mi nombre es *${name}*.\n\nTengo la siguiente consulta:\n"${message}"`;
    window.open(`https://wa.me/${resolvedPhone}?text=${encodeURIComponent(textFormatted)}`, '_blank');
    setMessage('');
  };
    ...
    >
      ...
      {isOpen && (
        ...
        >
          <div className="bg-ink-950 p-6 text-cream-50 flex items-center justify-between">
            ...
            <button onClick={() => setIsOpen(false)} className="p-1 hover:bg-ink-800 rounded-lg transition"><X size={16} /></button>
          </div>

          {/* CUERPO DEL CHAT */}
          <div className="p-5 bg-cream-50 h-72 overflow-y-auto flex flex-col gap-4">
            {step === 1 ? (
              <form onSubmit={handleInitChat} className="space-y-4 my-auto">
                <div className="text-center">
                  <p className="text-xs font-bold text-ink-900 mb-1">¡Bienvenido a Cirelia! 👋</p>
                  <p className="text-[10px] text-ink-500">¿Cómo podemos ayudarte hoy?</p>
                </div>
                <input 
                  type="text" required placeholder="Tu nombre..." value={name} onChange={(e) => setName(e.target.value)} 
                  className="w-full h-11 bg-cream-100 border border-ink-200 rounded-2xl px-4 text-xs outline-none focus:border-emerald-500 transition-colors" 
                />
                <button type="submit" className="w-full h-11 bg-ink-950 text-cream-50 rounded-2xl text-xs font-bold hover:bg-ink-800 transition-all active:scale-[0.98]">
                  Iniciar Chat
                </button>
              </form>
            ) : (
              <div className="flex flex-col h-full justify-between">
                <div className="space-y-4">
                  ...
                  <div className="flex gap-1 pl-8">
                    <span className="w-1 h-1 bg-ink-300 rounded-full animate-bounce [animation-delay:-0.3s]"></span>
                    <span className="w-1 h-1 bg-ink-300 rounded-full animate-bounce [animation-delay:-0.15s]"></span>
                    <span className="w-1 h-1 bg-ink-300 rounded-full animate-bounce"></span>
                  </div>
                </div>

                <form onSubmit={handleSendMessage} className="mt-4 flex gap-2 bg-cream-100 p-1.5 rounded-2xl border border-ink-200 focus-within:ring-2 focus-within:ring-emerald-500/20 transition-all">
                  <input 
      ...
      >
        ...
        <button 
          ...
          className="w-8 h-8 rounded-full bg-ink-950/20 hover:bg-ink-950/30 active:scale-95 flex items-center justify-center transition-all border border-white/10 shadow-[inset_0_1px_2px_rgba(255,255,255,0.1)]"
          title="Ocultar chat"
        >
